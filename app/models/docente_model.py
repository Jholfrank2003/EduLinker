...
def obtener_docentes():
    ...
    query = """
        ...
        GROUP BY u.id, u.nombre, u.apellido, u.correo, u.telefono, u.rol, u.estado, d.profesion
    """
...
def obtener_docente_por_id(docente_id):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    query = """
        SELECT u.*, d.id AS docente_pk, d.profesion
        FROM docente d
        INNER JOIN usuarios u ON d.usuario_id = u.id
        WHERE u.id = %s
    """
...
def actualizar_docente(docente_id, nombre, apellido, correo, telefono, rol, profesion, contrasena, asignaturas):
    cursor = mysql.connection.cursor()

    if contrasena:
        query = """
            UPDATE usuarios
            SET nombre=%s, apellido=%s, correo=%s, telefono=%s, rol=%s, contrasena=SHA2(%s,256)
            WHERE id=%s
        """
        cursor.execute(query, (nombre, apellido, correo, telefono, rol,  contrasena, docente_id))
    else:
        query = """
            UPDATE usuarios
            ...
            WHERE id=%s
        """
        cursor.execute(query, (nombre, apellido, correo, telefono, rol,  docente_id))
